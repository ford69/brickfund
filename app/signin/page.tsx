...
const OAUTH_ERROR_MESSAGES: Record<string, string> = {
  google_not_configured: 'Google sign-in is not configured. Please use email and password.',
  token_exchange_failed: 'Google sign-in failed. Please try again.',
  profile_failed: 'Could not load your Google profile. Please try again.',
  no_email: 'Your Google account has no email. Please use email sign-up instead.',
        ...
        <Card className="border border-border rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden bg-card/95 backdrop-blur-sm">
          ...
          <CardContent className="pt-0">
            {searchParams.get('reset') === 'success' && (
              <div className="mb-4 p-3 bg-green-500/10 border border-green-500/30 text-green-700 dark:text-green-400 rounded-lg text-sm">
            ...
            <div className="mt-6">
              ...
              <Button
                type="button"
                variant="outline"
                className="mt-6 w-full rounded-lg h-11 border-border"
                onClick={() => {
                  const redirect = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('redirect') : null;
